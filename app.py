import streamlit as st
from telethon import TelegramClient
import asyncio
import nest_asyncio

nest_asyncio.apply()

st.set_page_config(page_title="TeleBooks - تحميل الكتب", page_icon="📚")

# الحصول على بيانات API من secrets
api_id = int(st.secrets["api_id"])
api_hash = st.secrets["api_hash"]
phone = st.secrets["phone"]  # رقم الهاتف بدلاً من bot_token
channel_id = int(st.secrets["channel_id"])

# دالة البحث المحسّنة
def search_books(query):
    results = []
    try:
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        
        client = TelegramClient("user_session", api_id, api_hash)
        
        async def do_search():
            nonlocal results
            try:
                # تسجيل الدخول كمستخدم (بدلاً من بوت)
                await client.start(phone=phone)
                
                st.info("🔍 جاري الاتصال بالقناة...")
                
                # الحصول على القناة
                entity = await client.get_entity(channel_id)
                
                st.info(f"✅ تم الاتصال بـ: {entity.title}")
                
                # استخدام search الفعلي (متاح فقط لحسابات المستخدمين)
                st.info("📝 جاري البحث في القناة...")
                
                # البحث المباشر في القناة
                async for message in client.iter_messages(
                    entity, 
                    search=query,  # 🎯 هنا البحث الفعلي!
                    limit=None  # يمكن زيادتها حسب الحاجة
                ):
                    if message.file:
                        file_name = message.file.name or message.text or 'ملف'
                        
                        results.append({
                            'channel': entity.title,
                            'message_id': message.id,
                            'text': message.text or 'بدون وصف',
                            'file_name': file_name,
                            'size': message.file.size or 0,
                            'date': message.date,
                            'channel_id': entity.id,
                            'username': entity.username or 'no-username'
                        })
                
                if results:
                    st.success(f"✅ وجدت {len(results)} نتيجة!")
                else:
                    st.warning("⚠️ لم يتم العثور على ملفات تطابق البحث")
                    
            except Exception as e:
                st.error(f"❌ خطأ: {str(e)}")
                st.info("💡 تأكد من: 1) رقم الهاتف صحيح 2) channel_id صحيح 3) لديك وصول للقناة")
            
            await client.disconnect()
        
        loop.run_until_complete(asyncio.wait_for(do_search(), timeout=60))
        loop.close()
        
    except asyncio.TimeoutError:
        st.error("انتهت مهلة البحث. حاول مرة أخرى.")
    except Exception as e:
        st.error(f"حدث خطأ: {str(e)}")
    
    return results

# واجهة المستخدم
st.title("📚 TeleBooks - محرك البحث عن الكتب")
st.markdown("ابحث عن الكتب مباشرة في قناة تيليجرام")

# زر اختبار الاتصال
if st.button("🔧 اختبار اتصال الحساب بالقناة"):
    with st.spinner("جاري الاختبار..."):
        try:
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            client = TelegramClient("user_session", api_id, api_hash)
            
            async def test_connection():
                await client.start(phone=phone)
                try:
                    entity = await client.get_entity(channel_id)
                    st.success(f"✅ متصل بالقناة: **{entity.title}**")
                    
                    messages = await client.get_messages(entity, limit=1)
                    if messages:
                        st.info(f"✅ يمكن قراءة الرسائل - آخر رسالة: {messages[0].date}")
                    
                    st.success("🎉 الحساب يعمل بشكل كامل!")
                except Exception as e:
                    st.error(f"❌ فشل الاختبار: {str(e)}")
                
                await client.disconnect()
            
            loop.run_until_complete(test_connection())
            loop.close()
        except Exception as e:
            st.error(f"❌ خطأ: {str(e)}")

st.markdown("---")

# مربع البحث
query = st.text_input("🔍 ابحث عن كتاب:", placeholder="أدخل اسم الكتاب أو المؤلف")

# تهيئة session state
if 'results' not in st.session_state:
    st.session_state.results = []
if 'current_index' not in st.session_state:
    st.session_state.current_index = 0

if st.button("بحث"):
    if query:
        with st.spinner("جاري البحث في القناة..."):
            results = search_books(query)
            
            if results:
                st.session_state.results = results
                st.session_state.current_index = 0
    else:
        st.warning("الرجاء إدخال كلمة بحث")

# عرض النتائج
if st.session_state.results:
    result = st.session_state.results[st.session_state.current_index]
    
    st.markdown("---")
    st.subheader(f"📖 {result['file_name']}")
    st.write(f"**القناة:** {result['channel']}")
    st.write(f"**الوصف:** {result['text'][:300]}...")
    st.write(f"**الحجم:** {result['size'] / (1024*1024):.2f} MB")
    st.write(f"**التاريخ:** {result['date'].strftime('%Y-%m-%d %H:%M')}")
    
    # رابط مباشر محسّن
    if result['username']:
        st.markdown(f"### [📥 فتح في تيليجرام](https://t.me/{result['username']}/{result['message_id']})")
    else:
        channel_num = str(result['channel_id']).replace('-100', '')
        st.markdown(f"### [📥 فتح في تيليجرام](https://t.me/c/{channel_num}/{result['message_id']})")
    
    st.info(f"النتيجة {st.session_state.current_index + 1} من {len(st.session_state.results)}")
    
    col1, col2 = st.columns(2)
    
    with col1:
        if st.session_state.current_index > 0:
            if st.button("⬅️ السابق"):
                st.session_state.current_index -= 1
                st.rerun()
    
    with col2:
        if st.session_state.current_index < len(st.session_state.results) - 1:
            if st.button("➡️ التالي"):
                st.session_state.current_index += 1
                st.rerun()

# شريط جانبي
st.sidebar.title("⚙️ الإعدادات والمعلومات")
st.sidebar.info("""
**📚 عن التطبيق:**
بحث فعلي في قنوات تيليجرام باستخدام حسابك الشخصي

**🔑 المتطلبات:**
- api_id و api_hash من my.telegram.org
- رقم هاتفك المسجل في تيليجرام
- channel_id الصحيح

**💡 نصائح:**
- استخدم كلمات مفتاحية بسيطة
- البحث سيشمل جميع الرسائل
- النتائج مرتبة حسب التاريخ الأحدث
""")