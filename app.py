...
# ═══════════════════════════════════════════════════════════════
# 🔍 البحث الذكي (تم التعديل: الترتيب حسب طلب المستخدم)
# ═══════════════════════════════════════════════════════════════
...
def search_books_advanced(query, filters=None, limit=50):
    ...
    try:
        ...
        # 2. بناء الترتيب الذكي (Rank) حسب طلب المستخدم
        # الترتيب: 1. تطابق تام 2. يبدأ بالكلمة 3. يحتوي على الكلمة 4. يبدأ بالأحرف
        target_name_col = 'normalized_name' if 'normalized_name' in existing_columns else 'file_name'
        
        rank_clause = f"""
        CASE 
            WHEN {target_name_col} = ? THEN 0                                    -- 1. تطابق تام (الكتاب)
            WHEN {target_name_col} LIKE ? THEN 1                                 -- 2. يبدأ بالكلمة (الكتاب الناطق)
            WHEN {target_name_col} LIKE ? OR {target_name_col} LIKE ? THEN 2     -- 3. يحتوي على الكلمة (فن الكتاب)
            WHEN {target_name_col} LIKE ? THEN 3                                 -- 4. يبدأ بالأحرف (الكتابات)
            ELSE 4                                                               -- 5. يحتوي على الأحرف (الاستكتاب)
        END
        """
        
        rank_params = [
            clean_query,                # تطابق تام
            f"{clean_query} %",         # يبدأ بالكلمة + مسافة
            f"% {clean_query}",         # ينتهي بالكلمة
            f"% {clean_query} %",       # الكلمة في الوسط محاطة بمسافات
            f"{clean_query}%"           # يبدأ بالأحرف (الكتابات)
        ]

        full_params = rank_params + params + [limit]
        
        # الترتيب في SQL حسب الرتبة (match_rank) ثم طول الاسم
        ...
        # 3. الترتيب النهائي (Python) لدمج الأرقام (فن الكتاب 1 قبل فن الكتاب 2)
        if results:
            def smart_sort_key(row):
                text = row['file_name']
                # تقسيم النص لأرقام وحروف
                parts = [int(c) if c.isdigit() else c.lower() for c in re.split(r'(\d+)', text)]
                # الأولوية للرتبة، ثم للترتيب الطبيعي
