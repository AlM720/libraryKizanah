...
def search_books(query):
    ...
    try:
        ...
        # ✅ استخدام session_string بدلاً من phone
        client = TelegramClient(StringSession(session_string), api_id, api_hash)

        async def do_search():
            nonlocal results
            try:
                # ✅ الاتصال بدون طلب رقم هاتف أو رمز
                await client.start()

                st.info("🔍 جاري الاتصال بالقناة...")
                entity = await client.get_entity(channel_id)
                st.info(f"✅ تم الاتصال بـ: {entity.title}")

                st.info("📝 جاري البحث في القناة...")
                ...
                if results:
                    ...
                    st.warning("⚠️ لم يتم العثور على ملفات تطابق البحث")

            except Exception as e:
                st.error(f"❌ خطأ: {str(e)}")
                st.info("💡 تأكد من: 1) session_string صحيح 2) channel_id صحيح 3) لديك وصول للقناة")

            await client.disconnect()

        loop.run_until_complete(asyncio.wait_for(do_search(), timeout=60))
        loop.close()
...
# 💡 زر اختبار الاتصال (مُعدّل)
if st.button("🔧 اختبار اتصال الجلسة بالقناة"):
    with st.spinner("جاري الاختبار..."):
        try:
            ...
            async def test_connection():
                ...
                try:
                    entity = await client.get_entity(channel_id)
                    st.success(f"✅ متصل بالقناة: **{entity.title}**")
                    messages = await client.get_messages(entity, limit=1)
                    if messages:
                        st.info(f"✅ يمكن قراءة الرسائل - آخر رسالة: {messages[0].date}")
                    st.success("🎉 الجلسة تعمل بشكل كامل!")
                except Exception as e:
...
st.markdown("ابحث عن الكتب مباشرة في قناة تيليجرام")
...
if st.session_state.results:
    ...
    if result['username']:
        link = f"https://t.me/{result['username']}/{result['message_id']}"
    else:
        channel_num = str(result['channel_id']).replace('-100', '')
        link = f"https://t.me/c/{channel_num}/{result['message_id']}"

    st.markdown(f"### [📥 فتح في تيليجرام]({link})")
...
# 📎 الشريط الجانبي
st.sidebar.title("⚙️ الإعدادات والمعلومات")
st.sidebar.info("""
**📚 عن التطبيق:**
بحث فعلي في قنوات تيليجرام باستخدام جلسة حسابك المحفوظة.

**✅ المزايا:**
- لا حاجة لإدخال رمز تحقق
- يعمل على Streamlit Cloud بدون تفاعل مباشر

**🔑 المتطلبات:**
- `api_id`, `api_hash` من my.telegram.org
- `session_string` صالح
- `channel_id` صحيح (مع `-100`)
""")